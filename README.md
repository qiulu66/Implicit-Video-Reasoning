...
- Decoder Mode Loss ($\mathcal{L}_{\text{decode}}$): Trains the model to reconstruct the explicit CoT text ($Y_{\text{cot}}$) from the implicit tokens.
    $$
    \mathcal{L}_{\text{decode}} = -\sum_{t=1}^{|Y_{\text{cot}}|} \log P(y_t^{\text{cot}} | y_{<t}^{\text{cot}}, \text{pause}; \theta)
    $$
